...
artist. Letâ€™s say, itâ€™s *Straight Line Stitch* (they are great\!). A
little kicker for the
[morning](https://www.youtube.com/watch?v=4_5VAKdHMek).

## Packages

> Donâ€™t be afraid of the dark youâ€™re still held up by the stars
...
<https://www.musixmatch.com/> . Chose it because itâ€™s rather consistent,
and itâ€™s from Bologna, Italy (yeah\!).

The webiste offers the first 15 lyrics up front. That will do for the
moment (and fixing that is not that easy). Letâ€™s take a look
[here](https://www.musixmatch.com/artist/Straight-Line-Stitch#).

## Titles

First thing first, we would like to get a list of those title. Letâ€™s see
how.
...
OK. Itâ€™s a document. Thanks. And itâ€™s an XML document. Thatâ€™s sort of
html. Weâ€™ll handle it with `xml2` and `rvest`. Letâ€™s see a bit more of
that page.
...
To the browser\! Look at that â€œclassâ€ tags: they are *css selectors*,
...
are in the class â€œ.titleâ€. Letâ€™s see.
...
Thatâ€™s still quite a mess: we have too much stuff, such as some links
(called â€œhrefâ€) and more text than we need. Letâ€™s clean it up with
`html_text()`
...
Wundebar\! Now we have 15 song titles. But we want the lyrics\! Letâ€™s do
better.
...
It seems it works, letâ€™s do stor the result into the dataframe:
...
Cool, we donâ€™t gain much in terms of line of code, but it will be
usefull later\!

## And `purrr`\!

Cool, now we want to put grab all lyrics. Letâ€™s start with one at a
...
And letâ€™s grab the lyrics for that song. The content is marked by a css
selector called â€œp.mxm-lyrics\_\_contentâ€œ. Whatever\!
...
together with `glue`. As we are doing this, letâ€™s turn that flow into a
function:
...
The idea is to attribute to each word a score, expressing wether itâ€™s
more negative and positive, and then to sum up. To do this, we are going
to use Julia Silgeâ€™s *Tidytext* library and a *vocabulary* of words for
...
the words that are considered not interesting (they are called â€œstop
wordsâ€), stitch the dataframe to the scoress from afinn, and do the math
for each song.
...
```
    ...
    ## Don't crumble like paper macheâ€™
...
those scripts into functions. Letâ€™s try with a *A Tribe Called Red* and
*Angel Haze* (I picked them â€™cause they are great, and also because they
will show some limitations of the code Iâ€™m interested to tackle).

When we are about to do something over over, itâ€™s better to write
functions. So, letâ€™s do it\!

### Challenge

Another singer you should, should, should listen to is *Militia Vox*.
Try to replicate our work with her lyrics. Whatâ€™s the problem?

note: this is loosely inspired by Max Humberâ€™s
[post](https://www.r-bloggers.com/fantasy-hockey-with-rvest-and-purrr/)
and David Laingâ€™s post
